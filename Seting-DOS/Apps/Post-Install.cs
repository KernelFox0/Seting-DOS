/// 
/// Post-install script, Last modified: 2024. 06. 19.
/// 
/// Copyright (C) 2023-
/// 
...
namespace Seting_DOS.Apps
{
	public static class PostInstall
	{
		...
		{
			if (EnvVars.systemTheme == "black") { sel = 1; }
		...
		public static void Write(int sel = 0)
		{
			...
			else if (key.Key == ConsoleKey.Enter)
			{
				if (sel == 0)
				{
					...
					theme.Close();
					StreamWriter utheme = new StreamWriter(@"0:\Users\" + EnvVars.username + @"\theme.dat");
					utheme.Write(EnvVars.theme);
					utheme.Close();
					StreamWriter cn = new StreamWriter(@"0:\SDOS\preferences\compName.pref");
					cn.Write(EnvVars.hostname);
					cn.Close();
